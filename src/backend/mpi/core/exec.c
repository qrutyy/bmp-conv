...
static double mpi_process_by_rows(int rank, int size, const struct p_args *args, const struct filter_mix *filters)
{
	...
	if (!ctx.rank)
		log_info("Starting MPI process in CONV_COMPUTE_BY_ROW mode");
...
static double mpi_process_by_columns(int rank, int size, const struct p_args *args, const struct filter_mix *filters)
{
	...
	if (!ctx.rank)
		log_info("Starting MPI process in CONV_COMPUTE_BY_COLUMN (Transposed) mode");
...
double execute_mpi_computation(uint8_t size, uint8_t rank, struct p_args *compute_args, struct filter_mix *filters)
{
	...
	switch ((enum conv_compute_mode)compute_args->compute_cfg.compute_mode) {
	case CONV_COMPUTE_BY_ROW:
		total_time = mpi_process_by_rows(rank, size, compute_args, filters);
		break;
	case CONV_COMPUTE_BY_GRID:
		if (rank == 0)
			log_error("CONV_COMPUTE_BY_GRID mode is not implemented for MPI.");
		MPI_Abort(MPI_COMM_WORLD, 1);
		return -1.0;
	case CONV_COMPUTE_BY_PIXEL:
		if (rank == 0)
			log_error("CONV_COMPUTE_BY_PIXEL mode is not practical for MPI static distribution.");
		MPI_Abort(MPI_COMM_WORLD, 1);
		return -1.0;
