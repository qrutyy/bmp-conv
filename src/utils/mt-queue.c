...
static size_t estimate_image_memory(const bmp_img *img) {
	...
	return pixel_data_size + RAW_MEM_OVERHEAD;
}
...
static void queue_push(struct img_queue *q, bmp_img *img, char *filename)
{
	...
	while (q->size == MAX_QUEUE_SIZE || (q->current_mem_usage + image_memory > q->max_mem_usage && q->size > 0)) {
		pthread_cond_wait(&q->cond_non_full, &q->mutex); // unblocks threads with other roles (f.e. workers, that require some elements for convolution) and waits for return cond_non_full signal 
    }
