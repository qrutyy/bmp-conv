...
void queue_push(struct img_queue *q, bmp_img *img, char *filename)
{
    ...
    log_trace("Pushing '%s', estimated memory: %zu bytes. Current usage: %zu/%zu, size: %zu/%d",
             filename, image_memory, q->current_mem_usage, q->max_mem_usage, q->size, MAX_QUEUE_SIZE);
	...
	while (q->size >= MAX_QUEUE_SIZE || (q->current_mem_usage + image_memory > q->max_mem_usage && q->size > 0)) {
		log_debug("Queue full (size: %zu, mem: %zu + %zu > %zu). Waiting...",
                 q->size, q->current_mem_usage, image_memory, q->max_mem_usage);
...
restart_wait_loop:
	while (q->size == 0) {
		...
		if (__atomic_load_n(written_files, __ATOMIC_ACQUIRE) >= file_count) {
			log_debug("Pop termination check: written_files (%zu) >= file_count (%u). Returning NULL.",
