...
void *thread_function(void *arg) {
	...
	while (1) {
		switch (args->mode) {
		case BY_ROW:
			if (process_by_row(th_spec, &next_x_block, args->block_size,
							   &xy_block_mutex))
				goto exit;
			break;
		case BY_COLUMN:
			if (process_by_column(th_spec, &next_y_block, args->block_size,
								  &xy_block_mutex))
				goto exit;
			break;
		case BY_PIXEL:
		...
		case BY_GRID:
			if (process_by_grid(th_spec, &next_x_block, &next_y_block,
								args->block_size, &xy_block_mutex))
				goto exit;
			break;
		default:
			free(th_spec);
	...
	if (output_filename) {
		...
		if (threadnum > 1) 
			snprintf(output_filepath, sizeof(output_filepath),
				 "test-img/rcon_out_%s", args->input_filename);
		else 
			snprintf(output_filepath, sizeof(output_filepath),
