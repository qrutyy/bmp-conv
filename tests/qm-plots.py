...
try:
    ...
    print(f"Error: Results file '{RESULTS_FILE}' is empty or contains only comments/blank lines.")
    exit(1)
...
def plot_filter_bars(filter_type, base_df):
    ...
    if filter_df.empty:
        print(f"No data found for filter type '{filter_type}' with tags {tags_to_include}. Skipping plot.")
        return
