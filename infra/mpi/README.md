...
HYDRA - process manager of MPICH

```ENV HYDRA_IFACE=eth0```
forces MPICH to ignore localhost and use the Docker Bridge Network (eth0) to communicate. Now, when Node 1 phones home, it uses the correct IP address that connects it to the other containers.

MPI sends data (like "You are Rank 3") via environment variables over SSH. If PAM is on, it scrubs those variables, and the remote process forgets its rank, causing the "Rank 0 of 1" split-brain error. So turning PAM off ensures the environment variables sent by the Head Node actually reach the Worker Node.
